...
/**
 * Público. `/` é a porta de entrada do portfólio; `/api/health` é o alvo do cron
 * diário da Vercel que impede o banco gratuito de pausar — exigir sessão nele
 * derrubaria justamente a defesa contra inatividade.
 *
...
export function ehPublica(caminho: string): boolean {
  return PUBLICAS.some((p) => caminho === p || (p !== '/' && caminho.startsWith(`${p}/`)))
}
...
export function destinoSeguro(proximo: string | null | undefined): string {
  ...
  if (ehFormularioDeAuth(proximo.split('?')[0] ?? '')) return DESTINO_PADRAO
  return proximo
}
